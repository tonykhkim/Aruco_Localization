...
def imgmsg_to_cv2(img_msg):
  #img_msg=cv2.cvtColor(img_msg,CV_YUV2BGR_YUY2)
...
class image_converter:
  ...
  def __init__(self):
    self.pose_pub1 = rospy.Publisher("aruco/translation",Float32MultiArray,queue_size = 1)
  ...
  def callback(self,data):
    try:
      ...
      if np.all(ids is not None):   # If there are markers found by detector
        ...
        for i in range(0, len(ids)): # Iterate in markers
          ...
          #print(f"euler angles of a robot relative to marker{ids[i]} : {angles}")
          print(f"euler angles of a robot relative to marker{ids[i]} : {angle0}, {angle1}, {angle2}")
          #print('quaternion: ',quaternion)
          #print('t : ',t)  
          ...
          print(f"translation of a robot relative to marker{ids[i]} : {-t1},{t2},{t3}")
          #p1.position.x = -t1
          #p1.position.y = t2
          #p1.position.z = t3
          translation.data = [ids[i],-t1, t2, t3]
          ...
          rotation.data = [ids[i], angle0, angle1, angle2]
    ...
    try:
      self.pose_pub1.publish(translation) # After working with opencv image, convert to ros image again and publish.
...
if __name__ == '__main__':
  ...
  #with open('/home/patrol2/catkin_ws/src/ArUco_marker_detection/aruco/src/image_web/ost.yaml') as f: # ost.yaml : calibration again
